...
class MireviLab(Room):
    ...
    def run_story(self, user_items):
        ...
        print("You feel weirdly light. In the Mirevi Lab the floor is a bit more bouncy.")
        print("Why is that?")
        ...
        while True:
            ...
            if action == "yes":
                ...
                user_items.append("vr_headset", "A VR headset that takes you to a virtual world.", movable=True)
                self.run_virtual_world()
            else:
                print("You decide not to put on the VR headset.")
                print("Instead you look around for other clues.")
                print("Behind a shelf you find a red button that says 'Do not press!'.")
                action = input("Do you want to press the button? (yes/no): ").lower()
                if action == "yes":
                    print("You press the button. The lights flicker and you hear a click.")
                    print("Suddenly the room fills with a weird smell. You feel strangely intigued to dance your name.")
                    print("You have emberrased yourself in the Mirevi Lab.")
            return user_items
    ...
    def run_virtual_world(self):
        ...
        print("You walk towards it but it's locked. Next to it is a Keypad.")
